...
def randomize_train(d, n_patch=16, n_augment=1):
    da_geometric = D.DataAugment().flip().rot90().rotate(45).elastic_distortion(dist=35, scale=50)
    #da_geometric = D.DataAugment().flip().rot90().elastic_distortion(dist=15, scale=50)
    d = d.augment(da_geometric, N=n_augment).apply('mask', low_pass_thresh).random_patches(('raw,av'), patch_shape=758, n=n_patch, proba_map='mask')
    return d.map('raw','av')
...
def messidor_master():
    raw = D.images('/home/gaby/These/Data/Fundus/MESSIDOR/0-images/', 'raw', reshape=1500)
    av = D.images('/home/gaby/These/Data/Fundus/MESSIDOR/2-label_AV/', 'av', reshape=1500)
    return D.join(raw, av)

def messidor_val():
    raw = D.images('/home/gaby/These/Data/Fundus/MESSIDOR/0-images', 'raw', reshape=1500)
    av = D.images('/home/gaby/These/Data/Fundus/MESSIDOR/2-label_AV_val', 'av', reshape=1500)
    return D.join(raw, av)
...
def hrf_train():
    kwargs = dict(reshape=700, keep_proportion='pad')
    raw = D.images('/home/gaby/These/Data/Fundus/HRF/0-images/', 'raw', **kwargs).subset(start=15)
...
def hrf_test(reshape=True):
    kwargs = dict(reshape=700, keep_proportion='pad') if reshape else {}
    raw = D.images('/home/gaby/These/Data/Fundus/HRF/0-images/', 'raw', **kwargs).subset(stop=15)
    av = D.images('/home/gaby/These/Data/Fundus/HRF/2-label_AV/', 'av', **kwargs)
    return D.join(raw, av)

def DRIVE(train=True, preprocessing=True, vessel=True, av=False):
    if train:
        d = drive_train()
    else:
...
def MESSIDOR(preprocessing=True, vessel=True, av=False):
    d = messidor_clinic()
    #d = d.apply('av', _av2label, format=[{1:'red', 2:'blue', 3:'white'}])
    if preprocessing:
        d = d.apply({'pre':'raw'}, fundus_preprocessing, keep_parent=True, format='same')
    d=d.cache(ondisk='messidor.cache')
    return d

def HRF(train=True, preprocessing=True, vessel=True, av=False):
    if train:
        d = hrf_train()
    else:
        d = hrf_test()
...
def _av2label(av):
    label = np.zeros(av.shape[1:], np.int)
    label[av[0]>0.2] = 2
    label[av[2]>0.2] = 1
    label[av[1]>0.2] = 3
    return label
