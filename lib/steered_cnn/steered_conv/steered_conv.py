...
DEFAULT_STEERABLE_BASE = SteerableKernelBase.from_steerable(5, max_k=5)
...
class SteeredConvTranspose2d(nn.Module):
    def __init__(self, n_in, n_out=None, stride=2, padding='same', dilation=1, groups=1, bias=True,
        ...
        self.groups = groups
        if steerable_base is None:
            steerable_base = SteerableKernelBase.from_steerable(max(stride) if isinstance(stride, tuple) else stride)
        self.steerable_base = steerable_base
        if attention_base is True:
            attention_base = DEFAULT_ATTENTION_BASE
        elif attention_base is False:
    ...
    def forward(self, x, alpha=None, rho=None):
        ...
        out = self.steerable_base.conv_transpose2d(x, self.weights, alpha=alpha, rho=rho,
                                                   stride=self.stride, padding=self.padding, dilation=self.dilation)
