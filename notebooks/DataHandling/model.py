...
class HeavyResUNet(nn.Module):
    def __init__(self, n_in, n_out=1, p_dropout=0):
        super(HeavyResUNet, self).__init__()
        self.n_in = n_in
        self.n_out = n_out
        self.p_dropout = p_dropout
        ...
        # Down
        self.res1a = ResConvBlock(16, 32, depth=3)
        self.pool1v2 = ConvBN(2, 32, 64, stride=2, relu=True)
        self.res2b = ResConvBlock(64, depth=5)
        self.pool2v3 = ConvBN(2, 64, 128, stride=2, relu=True)
        self.res3c = ResConvBlock(128, depth=4)
        self.pool3v4 = ConvBN(2, 128, 256, stride=2, relu=True)
        self.res4d = ResConvBlock(256)

        # Up
        self.decode3 = ResConvBlock(256+128, 64)
        self.decode2 = ResConvBlock(2*64, 32)
        self.decode1 = ResConvBlock(2*32, 16)
...
class TallResUNet(nn.Module):
    def __init__(self, n_in, n_out=1, p_dropout=0):
        super(TallResUNet, self).__init__()
        self.n_in = n_in
        self.n_out = n_out
        self.p_dropout = p_dropout
        ...
        self.pool3v4 = ConvBN(2, 128, 256, stride=2, relu=True)
        self.res4d = ResConvBlock(256)
        self.pool4v5 = ConvBN(2, 256, 256, stride=2, relu=True)
        self.res5e = ResConvBlock(256)
        
        # Up
        self.decode4 = ResConvBlock(2*256, 128)
        self.decode3 = ResConvBlock(2*128, 64)
        self.decode2 = ResConvBlock(2*64, 32)
        self.decode1 = ResConvBlock(2*32, 16)
    ...
    def forward(self, x):
        ...
        x3 = self.res3c( self.pool2v3(x2) )
        x4 = self.res4d( self.pool3v4(x3) )
        x  = self.res5e( self.pool4v5(x4) )
        
        x = self.dropout(x)
        
        # Up
        y = F.interpolate(x, scale_factor=2)
        y = torch.cat((self.dropout(clip_center(x4, y.shape)), y), 1)
        y = self.decode4(y)
        
        y = F.interpolate(y, scale_factor=2)
        y = torch.cat((self.dropout(clip_center(x3, y.shape)), y), 1)
        y = self.decode3(y)
        
        y = F.interpolate(y, scale_factor=2)
        y = torch.cat((self.dropout(clip_center(x2, y.shape)), y), 1)
        y = self.decode2(y)
        
        y = F.interpolate(y, scale_factor=2)
        y = torch.cat((self.dropout(clip_center(x1, y.shape)), y), 1)
        y = self.decode1(y)
