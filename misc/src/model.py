...
class HemelingNet(nn.Module):
    def __init__(self, n_in, n_out=1, p_dropout=0, nfeatures_base=16, half_kernel_height=3):
        ...
        # --- MODEL ---
        if isinstance(nfeatures_base, int):
            n1 = nfeatures_base
            n2 = nfeatures_base * 2
            n3 = nfeatures_base * 4
            n4 = nfeatures_base * 8
...
class HemelingRotNet(nn.Module):
    def __init__(self, n_in, n_out=1, nfeatures_base=6, half_kernel_height=3,
                 ...
                 principal_direction='all', principal_direction_smooth=3):
    ...
    def forward(self, x):
        ...
        pDir1 = principal_direction(downSampledX,device=device,hessian_value_threshold=1,
                                    smooth_std=self.principal_direction_smooth)
        downSampledX = F.avg_pool2d(downSampledX, 2)
        pDir2 = principal_direction(downSampledX,device=device,hessian_value_threshold=1,
                                    smooth_std=self.principal_direction_smooth)
        downSampledX = F.avg_pool2d(downSampledX, 2)
        pDir3 = principal_direction(downSampledX,device=device,hessian_value_threshold=1,
                                    smooth_std=self.principal_direction_smooth)
        downSampledX = F.avg_pool2d(downSampledX, 2)
        pDir4 = principal_direction(downSampledX,device=device,hessian_value_threshold=1,
                                    smooth_std=self.principal_direction_smooth)
        downSampledX = F.avg_pool2d(downSampledX, 2)
        pDir5 = principal_direction(downSampledX,device=device,hessian_value_threshold=1,
                                    smooth_std=self.principal_direction_smooth)
